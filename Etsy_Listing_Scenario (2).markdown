...
Defining the Make.com scenario to process Google Sheet rows, generate unique listings, and publish to Etsy with proper error handling and idempotency.
...
## CategoryMap.json
Defining the taxonomy mappings for valid categories, ensuring listings are placed correctly.
...
```json
{
  "career_job_search": {
    ...
    "attributes_required": ["download_type", "subject", "file_type"],
    "attributes_defaults": {
      "download_type": "digital",
      "subject": ["career", "resume", "interview"],
      "file_type": ["PDF"]
    }
  },
  "wedding_vendor": {
...
## Content Prompt
Integrated into the Make.com scenario (module ID 13). Repeated here for clarity:

**System Prompt:**
You generate Etsy listing copy that is natural, persuasive, and unique. Never include location words. Title ≤ 140 chars.

**User Prompt:**
```json
Create listing content using this JSON:
{
...
## README.md
Guiding the setup, execution, and troubleshooting of the scenario.
...
- Format:
  ...
  ```
- Update templates to vary content while maintaining structure.

## Troubleshooting
- **Invalid Category**: Ensure `CategorySlug` matches keys in `CategoryMap.json`. Avoid `Books, Movies & Music` or `Other`.
- **Missing Attributes**: Check `Attr_*` columns or `CategoryMap.json` defaults.
- **Uniqueness Errors**: Adjust `VariantSeed` or template content to increase diversity.
- **Rate Limits**: Scenario includes 1–2s delays and retries for 429/5xx errors.
- **API Errors**: Verify `ETSY_API_KEY` and endpoint permissions.

## Notes
- Listings are created as Drafts by default. Review samples before setting `PublishStatus=Active`.
- Image packs must include at least 5 images, with one marked as a "DIGITAL DOWNLOAD" cover.
- Ensure OpenAI API key supports `gpt-4o` for content generation.
```